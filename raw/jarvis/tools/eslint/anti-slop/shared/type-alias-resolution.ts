...
type TypeScope = TSESTree.Node;

type TypeBinding = {
	readonly alias: TSESTree.TSTypeAliasDeclaration | null;
...
export type TypeAliasEnvironment = {
	readonly aliases: readonly TSESTree.TSTypeAliasDeclaration[];
...
export type ResolvedTypeMatcher = (
	...
	matches: (child: TSESTree.TypeNode) => boolean,
) => boolean;

const environmentsByProgram = new WeakMap<TSESTree.Program, TypeAliasEnvironment>();
...
function enclosingTypeScope(node: TSESTree.Node): TypeScope {
...
function declaredTypeBinding(node: TSESTree.Node): {
	readonly alias: TSESTree.TSTypeAliasDeclaration | null;
	readonly name: string;
} | null {
...
function collectTypeBindings(
	...
	aliases: TSESTree.TSTypeAliasDeclaration[],
): void {
	...
	// SAFETY: Visitor keys identify only ESTree child-node properties.
...
export function createTypeAliasEnvironment(
	...
	const aliases: TSESTree.TSTypeAliasDeclaration[] = [];
...
function ancestorDistance(ancestor: TSESTree.Node, node: TSESTree.Node): number | null {
...
export function resolvedTypeMatches(
	...
	const evaluate = (
		current: TSESTree.TypeNode,
		substitutions: Substitutions,
		resolvingAliases: ReadonlySet<TSESTree.TSTypeAliasDeclaration>,
	): boolean => {
